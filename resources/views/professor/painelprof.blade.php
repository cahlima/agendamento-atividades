            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="list-group mb-4">
                        ...
                        <a href="{{ route('professor.atividades.minhas') }}" class="list-group-item list-group-item-action">Ver minhas Atividades</a>
                        <a href="{{ route('professor.perfil.edit') }}" class="list-group-item list-group-item-action">Editar perfil</a>
                    </div>

                    <div>
                        <h4>Minhas Atividades</h4>
                        @if($minhasAtividades->isEmpty())
                            <p>Você ainda não está alocado em nenhuma atividade.</p>
                        @else
                            <ul class="list-group">
                                @foreach($minhasAtividades as $atividade)
                                    <li class="list-group-item">
                                        {{ $atividade->titulo }} - {{ \Carbon\Carbon::parse($atividade->data)->format('d/m/Y') }} às {{ $atividade->hora }}<br>
